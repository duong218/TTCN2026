...
const AddRoom = () => {
  ...
  const {axops, getToken} = useAppContext()
  ...
  const onSubmitHandler = async (e)=>{
    e.preventDefault()
    //kiểm tra xem tất cả các trường nhập liệu đã được điền đầy đủ chưa
    ...
    try {
      ...
      //converting amenities to array and keeping only enable amenities
      const amenities = Object.keys(inputs.amenities).filter(key => inputs.amenities[key])
      formData.append('amenities', JSON.stringify(amenities))

      // Thêm hình ảnh vào biểu mẫu dữ liệu
    ...
    <form onSubmit={onSubmitHandler}>
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        ...
        <button className='bg-primary text-white px-8 py-2 rounded mt-8 cursor-pointer' disabled={loading}>
           {loading ? 'Đang tạo thêm..': "Add Room"}
        </button>
    </form>
  )
}
