...
const RoomDetails = () => {
  ...
  //kiểm tra nếu phòng đó có sẵn
  const checkAvailablility = async ()=>{
    try {
      // kiểm tra xem ngày đặt lớn hơn ngày trả
      if(checkInDate >= checkOutDate){
        toast.error('Ngày đặt phải nhỏ hơn ngày trả')
        return;
      }
      ...
      if (data.success){
        if(data.isAvailable){
          ...
          toast.success('Phòng không có sẵn')
        }
      }else{
  ...
  //Hàm onSubmitHandler để kiểm tra khả dụng và đặt phòng
  const onSubmitHandler = async (e)=>{
    try {
      ...
      if(!isAvailable){
        ...
        const { data } = await axios.post('/api/bookings/book', {room: id, checkInDate, checkOutDate, guests, paymentMethod: "Thanh toán khách sạn"},{headers: { Authorization: `Bearer ${await getToken()}`}})
        if (data.success){
  ...
  return room && (
    <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
      ...
      <div className='flex flex-col md:flex-row items-start md:items-center gap-2'>
        <h1 className='text-xl md:text-xl font-playfair'>
      ...
      <div className='flex flex-col md:flex-row md:justify-between mt-10 '>
          <div className='flex flex-col'>
            <h1 className='text-2xl md:text-3xl font-playfair'>Experience Luxury Like Never Before</h1>
          ...
          <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
            ...
            <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-4 md:gap-10 text-gray-500'>
                ...
                <div className='flex flex-col'>
                  ...
                  <input onChange={(e)=>{setGuest(e.target.value)}} value={guests} type="number" id='guests' placeholder='1' className='max-w-20 rounded border border-gray-300 px-3 py-2 mt-1.5 outline-none' required/>
                </div>

            </div>

              <button type='submit' className='bg-primary hover:bg-primary-dull active:scale-95 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-25 py-3 md:py-4 text-base cursor-pointer'>
                {isAvailable ? "Đặt ngay" : "Kiểm tra phòng có sẵn"}
              </button>
          </form>
